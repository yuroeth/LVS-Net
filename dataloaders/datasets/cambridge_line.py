...
from dataloaders import custom_transforms_line as tr
...
class CambridgeSegmentation(Dataset):
    ...
    def __init__(self,
        ...
        id2centers = json.load(
            open(osp.join(self._data_dir, "id2lines.json")))
        ...
        try:
            ...
            if osp.exists(self.seg_target_list[index]):
                _seg_target = cv2.imread(
                    self.seg_target_list[index])        # pixel (1920, 1080)
                _seg_target = _seg_target[::2, ::2, :]  # pixel (960, 540)
            else:
